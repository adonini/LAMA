...
def test_add_member_with_temporary_duty_cf_early_above_6_ends_member_valid_duty_option_2(client):
    ...
    payload["end_date"] = datetime(2024, 5, 5).date().isoformat()
    print(payload)
    response = client.post(url, data=payload)
    assert response.status_code == 200
    assert MembershipPeriod.objects.filter(member=member).first().end_date == datetime(2024, 5, 5).date()
    assert member.current_cf() is None
    assert member.common_found.order_by("-start_date").first().end_date == datetime(2024, 5, 5).date()
    autorship = AuthorshipPeriod.objects.filter(member=member).first()
    assert autorship.end_date == datetime(2024, 11, 5).date()
    assert member.dated_authorship(datetime(2024, 11, 5).date()) is not None
    assert member.dated_authorship(datetime(2024, 11, 6).date()) is None
