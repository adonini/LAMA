...
def test_add_member_with_temporary_duty_cf_early_above_6(client):
    ...
    assert member.dated_authorship(datetime(2024, 11, 6).date())
    assert not member.dated_authorship(datetime(2025, 8, 5).date())
