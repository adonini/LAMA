...
def test_add_member_with_temporary_duty_cf_early_above_6_permanent_late(client):
    ...
    payload = {
        ...
        "is_cf": 'on',
        "cf_start": datetime(2023,2,10).date().isoformat(),
        "role": "affiliated",
        "new_duties": json.dumps([
            {
                "duty": duty.id,
                "start_date": datetime(2023,8,15).date().isoformat(),
                "end_date": datetime(2023,9,9).date().isoformat(),
            }
        ]),
    }
    ...
    duty_payload = {
        "id": member.id,
        "duty": duty2.id,
        "start_date" : datetime(2024, 4, 11).date().isoformat(),
    }
