...
def test_add_member_with_temporary_duty_cf_early_above_6_ends_cf_end_date_before_auth(client):
    ...
    payload['is_cf'] = 'off'
    payload['cf_end'] = datetime(2024, 5, 15).date().isoformat()
    response = client.post(url, data=payload)
    assert response.status_code == 200
    autorship = AuthorshipPeriod.objects.filter(member=member).first()
    print(autorship.end_date)
    assert autorship.start_date == datetime(2023, 10, 15).date()
    assert autorship.end_date == datetime(2024, 11, 15).date()
