...
def test_add_member_with_permanent_duty_and_temporary_cf_early_above_6_institute_chage_cf_unticked_duty_ended_order_2(client):
    ...
    assert MemberDuty.objects.filter(member=member, duty=duty).first().end_date == datetime(2025,3,25).date()
    ...
    assert member.current_cf().end_date == datetime(2025,4,11).date()-relativedelta(days=1)
    assert member.membership_periods.order_by("-start_date").first().start_date == datetime(2025,4,11).date()
    assert MemberDuty.objects.filter(member=member, duty=duty).exists()
    assert CommonFound.objects.filter(member=member).exists()
    assert CommonFound.objects.filter(member=member).order_by("-start_date").first().end_date == datetime(2025,4,11).date()-relativedelta(days=1)
